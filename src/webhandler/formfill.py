...
class SharePointFormFill():
    ''' Class to check recursively folders to fill sharepoint form '''
    ...
    def __init__(self) -> None:
        ...
        self.DATAFRAME = get_dataframe_from_excel(excel_path)
        self.MAX_DEPTH = int(str(CONFIGS.get('PROFUNDIDADE', '0')))

    def fill_form(self, current_folder: str) -> None:
        ...
        for file_or_dir in items:
            ...
            try:
                ...
                medicao = None
            orderdt = (
                self.DATAFRAME[self.DATAFRAME['Num. da Medição'].isin([medicao]) | self.DATAFRAME['Origem'].isin([orderid])]
                if medicao is not None else
                self.DATAFRAME[self.DATAFRAME['Origem'].isin([orderid])]
            )
            if orderdt.empty:
    ...
    def recursive_search(self, current_folder: str, cur_depth: int) -> None:
        ''' Function to search recursivily to folders to find documents to send '''
        if cur_depth > self.MAX_DEPTH:
            return
        if cur_depth == self.MAX_DEPTH:
            self.fill_form(current_folder)
            return
        items = os.listdir(current_folder)
        for item in items:
            current_item = os.path.join(current_folder, item)
            if os.path.isdir(current_item):
                self.recursive_search(current_item, cur_depth + 1)

    def sharepoint_fillform(self) -> None:
        ...
        self.recursive_search(UPOFOLDER, 1)
